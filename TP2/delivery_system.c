...
void* funcion_cocinero(void* arg) {
    ...
    while (1) {
        ...
        printf("[Cocinero  %d] Tomó pedido #%d (%s)\n", id, p.id, p.tipo_comida);
        pthread_mutex_unlock(&mtx_pend);
...
void* funcion_repartidor(void* arg) {
    ...
    while (1) {
        ...
        printf("[Repartidor %d] Retiró pedido #%d (%s) para entrega\n",
               id, p.id, p.tipo_comida);
        pthread_mutex_unlock(&mtx_list);
        ...
        pthread_mutex_lock(&mtx_global);
        printf("[Repartidor %d] Pedido #%d entregado con éxito ✓  (%d/%d)\n",
...
int main(void) {
    ...
    printf("\n=== Simulación completada. %d pedidos entregados. ===\n", pedidos_entregados);
    return 0;
}
